...
class Coffeeapi {
private:
    std::string api_base = "https://coffee.alexflipnote.dev";
    ...
    http_request create_request(const std::string& endpoint, const std::string& method, const std::string& data = "") {
        ...
        request.headers().add(U("Host"), U("coffee.alexflipnote.dev"));
...
public:
    ...
    pplx::task<json::value> get_random_coffee() {
        return make_api_call("/random.json","GET");
    }
};
